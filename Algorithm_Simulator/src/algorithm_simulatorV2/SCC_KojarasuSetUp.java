...
/**
 * set up the window for simulating SCC_kojarasu
 *
 * @author Arnab Sen Sharma
 */
public class SCC_KojarasuSetUp {
    ...
    static int a_cnt = 0;
    static Polygon[][] gon_arr = new Polygon[101010][2];
    ...
    static int stateCnt;
    boolean visibility[][] = new boolean[10101][2];
    int currState = 0;
    ...
    SCC_KojarasuSetUp() {
        ...
        primaryStage.setTitle("SCC(Double DFS appraoch or Kojarasu method) Simulator");
        ...
        addDirEdge.setOnAction(e -> {
            ...
            GetDirEdgeWithRev ge = new GetDirEdgeWithRev(rect, circle, root, co_ordinX, co_ordinY, text, vec, nodetoline, line);
        });
        ...
        run.setOnAction(e -> {
            if (cnt > 0) {
                ...
                for (int i = 1; i <= a_cnt; i++) {
                    gon_arr[i][0].setVisible(true);
                    gon_arr[i][1].setVisible(false);
                    visibility[i][0] = true;
                ...
                RunSCC_kojarasu ob1 = new RunSCC_kojarasu(circle, dfn, finding, exiting, 1, vec, line, state, nodetoline, visibility);
                ShowState(0);
            }
        });
        clear.setOnAction(e -> {
        ...
        /*pause.setId("button_pause");
         ...
         clearButtonPressed();
         if (step == 1) {
         //ob1 = new Thread(new RunSCC_kojarasu(circle, dfn, finding, exiting, 1, vec));
        ...
        back2.setOnAction(e -> {
            grid.getChildren().clear();
            grid.getChildren().addAll(addNode, addDirEdge, run, clear, back);
            clearButtonPressed();
            for (int i = 1; i <= cnt; i++) {
                circle[i].setFill(Color.RED);
            }
        ...
        prev.setPrefSize(190, 50);
        next.setPrefSize(190, 50);
        back2.setPrefSize(190, 50);
        ...
        VBox run_panel = new VBox();
        InstructionPanel.setPrefSize(width - 150, 100);
        InstructionPanel.setId("vbox_Instruction");
        run_panel.setId("vbox_Instruction");
        Instruction.setId("text");
        InstructionPanel.getChildren().addAll(Instruction);
        vvbb.getChildren().addAll(root, InstructionPanel);

        HBox hb = new HBox();
        hb.getChildren().addAll(vvbb, grid);

        Scene scene = new Scene(hb, width + 200, 600);
    ...
    void ShowState(int index) {
        ...
        for (int i = 1; i <= a_cnt; i++) {
            gon_arr[i][0].setVisible(state[index].visibility[i][0]);
            gon_arr[i][1].setVisible(state[index].visibility[i][1]);
        }
        updateButtonVisibility();
    }
